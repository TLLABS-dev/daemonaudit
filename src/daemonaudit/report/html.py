...
def render_html(report: ScanReport) -> str:
    ...
    w("<h2>Checks</h2><div class=\"wrap\"><table><tr><th>id</th><th>status</th><th>check</th><th>note</th></tr>")
    for r in report.results:
        note = (r.note or "").strip().splitlines()
        w(f'<tr><td class="id">{e(r.check_id)}</td><td class="st {e(r.status)}">{e(r.status)}</td><td>{e(r.title)}</td><td>{e(note[-1] if note else "")}</td></tr>')
    w("</table></div>")
