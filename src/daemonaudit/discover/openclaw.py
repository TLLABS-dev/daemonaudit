...
# A real OpenClaw gateway: node running the openclaw entry (openclaw.mjs / the `openclaw` bin
# shim / `openclaw-gateway` wrapper) with the `gateway` command on its line.
...
def config_path(home: Path) -> Path:
    ...
    if env:
        ...
        # only honour it for the home it belongs to; scanning a backup must not read the live config
        if p.parent == home or not (home / "openclaw.json").exists():
            return p
...
def _belongs_to(proc: dict, home: Path, plat: Platform) -> bool:
    """Attribute a gateway process to *this* home only (same rule as Hermes: a process we
    cannot attribute is not ours — scanning a backup must never probe the live daemon)."""
...
def build_layout(home: Path, cfg: dict, pkg_root: Path | None, included: list[Path] = ()) -> Layout:
    ...
    return Layout(
        ...
        sprawl_paths=["agents", "sessions", "transcripts", "logs", "state", "tui", "cron", "memory", "audit", "hooks.json", "exec-approvals.json",
                      "openclaw.json.bak"] + [str(w) for w in workspaces],
...
def discover_openclaw(plat: Platform, home_override=None) -> Target | None:
    ...
    procs = [p for p in plat.find_processes("openclaw") if _is_openclaw_proc(p) and _belongs_to(p, home, plat)]
    ...
    t.layout = build_layout(home, s.cfg, pkg, s.included)
    return t
